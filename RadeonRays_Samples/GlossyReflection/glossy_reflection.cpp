...
std::vector<Light> PrepareLights(int count)
{
    std::vector<Light> lights;
    for (int a = -(int)count/2; a <= (int)count/2; ++a)
    {
        Light light;
        light.position = float3(a * 10.f, 800.f, 0.f);
        light.intencity = float3(10000.f, 10000.f, 8000.f);
...
/*    if (argc != 1)
    {
        std::cerr << "Usage: " << argv[0] << " <light count>"<< std::endl;
        return -1;
    }*/
    ...
    int light_count = 1;// atoi(argv[1]);
    int w = 1920;
    int h = 1080;
    int shadow_rays_per_frame = w * h;
    int initial_rays_count = w * h; // 1 ray per pixel

    std::vector<Light> lights = PrepareLights(light_count);

    std::cout << "Primary rays: " << initial_rays_count << std::endl;
    std::cout << "Max shadow rays per frame: " << shadow_rays_per_frame << " (" << light_count << " lights)" << std::endl;
    ...
    CLWProgram program = CLWProgram::CreateFromFile("glossy_reflection.cl", options.c_str(), context);
    ...
    for (int a = 0; a < frame_count; ++a)
    {
        ...
        //Trace shadow rays
        uint32_t shadow_rays_count = primary_rays_buffer.GetElementCount();
        ...
        {
            //Shade and generate new rays
            CLWKernel kernel = program.GetKernel("ShadeIndirectRays");
            int argid = 0;
            kernel.SetArg(argid++, shapes_buffer);
            kernel.SetArg(argid++, vertex_buffer);
            kernel.SetArg(argid++, index_buffer);
    ...
    SaveImage("glossy_reflection.jpg", image_data.get(), w, h);
}
